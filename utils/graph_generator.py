"""
Generadores de hipergrafo a partir de trayectorias de usuarios.

Conceptos:
- Cada usuario es un nodo. Cada hiper-arista representa (localización, ventana
  temporal). Se construye un índice esparso usuario→hiper-arista.
"""
...
def hypergraph_generator(traj, unique_len=None):
    """Construye índice de hiper-aristas para una matriz de trayectorias.

    traj: np.array [num_usr, T]
    unique_len: tamaño de ventana; si None, usa únicos del total.
    return: np.array shape (2, num_incidentes), dict (loc,t)->edge_id si usa ventanas
    """
    hyperedge_index = [[], []]
...
def hypergraph_sequence_generator(traj, seq_num, device, unique_len=48):
    """Divide la trayectoria en `seq_num` segmentos y construye hipergrafo por segmento."""
...
def hypergraph2hyperindex(hypergraph,device):
    """Convierte matriz de incidencia H densa a índice esparso (nodos, hiper-aristas)."""
    hyperedge_index = [[], []]
...
def edge2h(edge):
    """Convierte índice esparso (2, E) a matriz H densa."""
...
def construct_network(trace_array):
    """Construye red simple de co-ocurrencia (nodos se conectan si comparten hiper-arista)."""

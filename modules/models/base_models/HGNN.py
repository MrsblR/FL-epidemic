"""
Operadores de hipergrafo con soporte para privacidad diferencial.

Componentes:
- Hcov_node2edge / Hcov_edge2node: pasos de mensaje simples entre nodos y
  hiper-aristas (usados como referencia/variantes).
- Dp_HypergraphConv: versión basada en PyG con pipeline nodo→hiper-arista→nodo,
  normalizaciones D y B. Opcionalmente aplica ruido DP (Dp_Agg) tras el primer
  paso cuando loc_dp está activo y epoch>0. Puede inyectar embeddings macro
  de región por hiper-arista.
Notas: Se modifican atributos internos para acomodar macro_emb; evitar depender
de out_channels mutable fuera de este módulo.
"""
...
class Dp_HypergraphConv(MessagePassing):
    ...
    def forward(self, x: Tensor, hyperedge_index: Tensor,
        ...
        # Proyección lineal inicial de nodos
        x = self.lin(x)
        ...
        # Agregación nodo→hiper-arista
        out = self.propagate(hyperedge_index, x=x, norm=B, alpha=alpha,
                             size=(num_nodes, num_edges))
        # Inyección de ruido DP en representaciones de hiper-aristas
        ...
        if self.concat is True:
            ...
            out = out.mean(dim=1)
        # Restablece dimensión de salida base usada por el modelo

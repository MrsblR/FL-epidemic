"""
Modelo principal MultiScaleFedGNN.

Estructura:
- usr_emb: embedding aprendible por usuario (nodos del hipergrafo).
- hyper_convs: dos capas de convolución sobre hipergrafo (ver HGNN).
- RNN (LSTM): integra en el tiempo las representaciones por segmento.
- output_layer: proyección final a clases.

Privacidad:
- Si loc_dp: se pasan ubicaciones reales/falsas a cada capa para añadir ruido
  DP en la agregación de hiper-aristas (ver Dp_HypergraphConv).
- Si macro: se inyecta embedding regional por hiper-arista.
"""
...
class MultiScaleFedGNN(nn.Module):
    ...
    def forward(self, hyperedge_seq, epoch, loc_graph_idx=None):
        """
        Aplica dos capas de HGNN por cada segmento temporal de hiper-aristas,
        concatena la secuencia y la procesa con LSTM. Devuelve logits.
        """
        # Representación inicial de usuarios
        ...
        for idx in range(len(hyperedge_seq)):
            ...
            for hyper_conv in self.hyper_convs:
                ...
                if self.macro:
                    ...
                    macro_emb = None
                # Convolución sobre hipergrafo con posible ruido DP en agregación

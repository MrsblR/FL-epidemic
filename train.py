"""
Entrenamiento de Falcon (aprendizaje federado con hipergrafos) para
predicción individual de infección preservando la privacidad.

Flujo general:
- Carga configuración y datos (trayectorias y etiquetas por usuario).
- Genera secuencia de hiper-aristas espaciotemporales a partir de trayectorias.
- (Opcional) Prepara embeddings macro por región y ubicaciones falsas plausibles.
- Define el modelo MultiScaleFedGNN (HGNN + RNN).
- Entrena con CrossEntropy; registra métricas y curvas PR.
- Privacidad:
  * DP de localización: ruido en agregación de hiper-aristas y clipping en usr_emb.
  * DP federado: ruido gaussiano a parámetros (excepto usr_emb) por época.
"""
...
# Dispositivo: CPU si debug; CUDA por defecto.
...
device_desc = torch.cuda.get_device_name(device) if device.type == "cuda" and torch.cuda.is_available() else "CPU"
print(f"[config] dataset={env_args.get('dataset')} epi_dyna={env_args.get('epi_dyna','?')} | device={device_desc} | tensorboard={fun_args['tensorboard']} | DP(loc={model_args['loc_dp']}, fl={model_args['fl_dp']})")
...
# Carga de datos epidemiológicos (trayectorias por usuario y etiquetas)
...
# Generación de secuencia de hipergrafos (loc,t) como hiper-aristas
...
# (Opcional) Embeddings macro por región para inyectar señales regionales
if model_args['macro']:
    """
    inputs: shape ()
    return: shape ()
...
# (Opcional) Generación de ubicaciones/Trayectorias falsas plausibles para DP
...
# Definición del modelo (HGNN secuencial + RNN temporal)
...
def train_model(model, criterion, optimizer, scheduler, num_epochs, print_freq=10):
    """Entrena y evalúa el modelo, aplicando DP cuando corresponde."""
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            with torch.set_grad_enabled(phase == 'train'):
                # Pase hacia delante: HGNN por cada segmento temporal + RNN
                ...
                if phase == 'train':
                    ...
                    optimizer.step()
                    scheduler.step()
                    # Clipping de actualización en usr_emb (limita sensibilidad)
                    if cfg['model_args']['loc_dp']:
                    ...
                    # Ruido DP federado a parámetros (excepto usr_emb)
                    if cfg['model_args']['fl_dp']:
                        fl_dp(model, optimizer, **cfg['model_args'])

            # Métricas: curva PR, F1 máximo, AUC, ACC con umbral de F1

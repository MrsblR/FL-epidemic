...
## Nota (ES)
- El entrenamiento aplica mecanismos de Privacidad Diferencial a nivel de
  estructura (hiper-aristas) y a nivel de parámetros (servidor), controlables
  en `config.json` mediante `model_args.loc_dp` y `model_args.fl_dp` y sus
  parámetros (`*_eps`, `*_delt`, `*_clip`).
- Para activar la inyección de señales regionales (modelo macro), establezca
  `"macro": true` y asegúrese de que los archivos `region_epi_emb*.npy` estén
  presentes en la carpeta del dataset correspondiente.
